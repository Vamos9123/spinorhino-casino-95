...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-6060f1aa";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-6060f1aa">'+'<span class="tile-face-6060f1aa tile-front-6060f1aa"></span>'+'<span class="tile-face-6060f1aa tile-back-6060f1aa"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-6060f1aa")){return;}
var back=tile.querySelector(".tile-back-6060f1aa");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-6060f1aa");if(isMine){tile.classList.add("boom-6060f1aa");}
if(dim){tile.classList.add("dimmed-6060f1aa");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-6060f1aa")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-6060f1aa");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-6060f1aa")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-6060f1aa");void panelEl.offsetWidth;panelEl.classList.add("jolt-6060f1aa");}
endRound();}
